# 🎮 Tic Tac Toe Game (Java Swing)

## 📌 Project Title

**Tic Tac Toe GUI Game using Java**

## 📝 Introduction

This project is a GUI-based Tic Tac Toe game developed using Java
Swing.\
It allows two players (X and O) to play the game on a 3×3 grid with
real-time status updates, automatic winner detection, and score
tracking.\
The interface is simple, interactive, and visually appealing.

## 🚀 Features

-   2-player gameplay (X vs O)\
-   Interactive GUI using Java Swing\
-   Automatic turn switching\
-   Winner detection with highlighted winning cells\
-   Draw detection\
-   Score tracking (X Wins, O Wins, Draws)\
-   Reset/New Game button\
-   Color-based UI for better experience

## 🧱 Project Structure

    TicTacToeGUI.java
    README.md

## 🛠 Technologies Used

-   Java (JDK 8 or above)\
-   Java Swing\
-   AWT (Layout and Event Handling)

## 📚 Concepts Used

-   OOP (Classes & Objects)\
-   Event Handling (ActionListener)\
-   GUI Components (JFrame, JButton, JPanel, JLabel)\
-   2D Arrays for game board\
-   Condition checks for winner and draw\
-   Methods and modular design

## ▶️ How to Run the Project

### Using Command Line

    javac TicTacToeGUI.java
    java TicTacToeGUI

### Using IDE

1.  Create a new Java project\
2.  Add the file `TicTacToeGUI.java`\
3.  Run the main() method

## 🎮 How to Play

1.  Player X starts\
2.  Players click empty cells to mark\
3.  Align three marks to win\
4.  Full board with no winner = Draw\
5.  Click New Game to restart

## 🎨 UI Highlights

-   X: Blue\
-   O: Red\
-   Winning cells: Light Green\
-   Status bar updates score and turn

## 🔄 Future Improvements

-   Single-player AI\
-   Sound effects\
-   Online multiplayer\
-   Timer for moves

## 🧑‍💻 Author

Dikshit Raj

## 📄 License

Free to use for learning purposes.
